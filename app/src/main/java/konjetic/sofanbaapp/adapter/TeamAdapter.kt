    ...
    override fun onBindViewHolder(holder: TeamViewHolder, position: Int) {
        ...
        holder.binding.teamFavoriteIcon.setOnClickListener {
            if(holder.binding.teamFavoriteIcon.isActivated){
                ...
                runBlocking { NBADatabase.getDatabase(context)?.getNBADao()?.insertFavoriteTeam(
                    TeamData(team?.id!!, team.abbreviation, team.city, team.conference, team.division, team.full_name, team.name, if(favList.size > 0){favList[favList.size-1].position+1}else{0})
                ) }
                holder.binding.teamFavoriteIcon.isActivated = true
            }
        }

...
class TeamMatchPagingSource(private val service1: BallDontLieService, val teamId: Long) : PagingSource<Int, MatchResponseData>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, MatchResponseData> {
        return try {
            val meta = service1.getGamesOfTeam(teamId, false)
            val nextPageNumber = params.key ?: meta.meta.total_pages
            val response = service1.getGamesOfTeamPaged(teamId, 25, nextPageNumber, false)
